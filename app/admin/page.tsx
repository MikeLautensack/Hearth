...
export default async function Admin() {
  ...
  const pendingUsers = allUsers?.filter(u => u.access_status === "pending") || [];
  const approvedUsers = allUsers?.filter(u => u.access_status === "approved") || [];
  const deniedUsers = allUsers?.filter(u => u.access_status === "denied") || [];
      ...
      <main className="mx-auto max-w-4xl px-4 py-24">
        ...
        <div className="mb-8">
          ...
          <p className="mt-2 text-gray-400">
            Manage access requests and users for Mike&apos;s Hearth.
          </p>
        </div>
        ...
        <div className="mb-8 grid gap-4 md:grid-cols-3">
          ...
          <div className="rounded-xl border border-red-500/30 bg-red-500/10 p-4">
            <p className="text-3xl font-bold text-red-500">{deniedUsers.length}</p>
            <p className="text-sm text-gray-400">Denied Users</p>
          </div>
        </div>

        {/* Pending Requests */}
        <section className="mb-8">
          <h2 className="mb-4 text-xl font-bold text-white">Pending Requests</h2>
          {pendingUsers.length > 0 ? (
            <AdminUserList users={pendingUsers as Profile[]} adminId={user.id} />
          ) : (
        ...
        {deniedUsers.length > 0 && (
          <section>
            <h2 className="mb-4 text-xl font-bold text-white">Denied Users</h2>
            <AdminUserList users={deniedUsers as Profile[]} adminId={user.id} showDenied />
          </section>
        )}
      </main>
    </div>
